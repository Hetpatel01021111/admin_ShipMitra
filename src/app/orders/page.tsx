...
const statusFilters = [
    { value: "all", label: "All Active", icon: CircleDot, color: "text-gray-600" },
...
function OrdersContent() {
    ...
    const handleStatusFilterChange = (newStatus: string) => {
        ...
        if (newStatus === "all") {
            ...
            router.push(`/orders?status=${newStatus}`);
        }
    };
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter((order) => {
            ...
            if (statusFilter === "all") {
                // "All" in main Orders view = only active orders (not delivered or cancelled)
                matchesStatus = order.status !== "delivered" && order.status !== "cancelled";
            } else if (statusFilter === "in_transit") {
                // In Transit includes multiple sub-statuses
                matchesStatus = order.status === "in_transit" || order.status === "picked_up" || order.status === "pickup_scheduled";
            } else {
                // Specific status filter (delivered, cancelled, pending)
                                                        ...
                                                        {(order as any).riderName ? (
                                                            <div className="flex items-center gap-2">
                                                                <div className="w-6 h-6 rounded-full bg-green-600 flex items-center justify-center text-white text-xs font-bold">
                                                                    {(order as any).riderName.charAt(0)}
                                                                </div>
                                                                <span className="text-sm font-medium">{(order as any).riderName}</span>
                                                            </div>
                                                        ) : (
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {order.purchaseDetails ? (
                                                            <div className="flex flex-col">
                                                                <span className="text-sm font-medium text-gray-900">{order.purchaseDetails.billNumber}</span>
                                                                <span className="text-xs text-gray-500">{formatCurrency(order.purchaseDetails.totalAmount)}</span>
                                                            </div>
                                                        ) : (
