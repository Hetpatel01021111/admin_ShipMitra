...
function GSTContent() {
    ...
    const calculateGST = () => {
        ...
        const byMonth: Record<string, { sales: number; inputTax: number; count: number }> = {};
        ...
        orders.forEach((order) => {
            ...
            if (!byMonth[monthKey]) {
                byMonth[monthKey] = { sales: 0, inputTax: 0, count: 0 };
            }
            byMonth[monthKey].sales += order.amount || 0;
            // Add input tax from linked purchase bills
            if (order.purchaseDetails?.gstAmount) {
                byMonth[monthKey].inputTax += order.purchaseDetails.gstAmount;
            }
        ...
        const gstResults: GSTData[] = Object.entries(byMonth)
            .map(([month, data]) => {
                ...
                return {
                    ...
                    netPayable: totalTax - data.inputTax,
    ...
    const totalStats = {
        ...
        totalInputTax: gstData.reduce((sum, g) => sum + g.inputTax, 0),
        totalNetPayable: gstData.reduce((sum, g) => sum + g.netPayable, 0),
        totalInvoices: gstData.reduce((sum, g) => sum + g.invoiceCount, 0),
    };

    const downloadGSTR = () => {
        let csvContent = "Month,Total Sales,CGST (9%),SGST (9%),IGST (18%),Total Output Tax,Input Tax Credit,Net Payable,Invoice Count\n";
        gstData.forEach((g) => {
            csvContent += `${g.month},${g.totalSales.toFixed(2)},${g.cgst.toFixed(2)},${g.sgst.toFixed(2)},${g.igst.toFixed(2)},${g.totalTax.toFixed(2)},${g.inputTax.toFixed(2)},${g.netPayable.toFixed(2)},${g.invoiceCount}\n`;
        });
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
                            <CardContent className="p-4">
                                ...
                                <p className="text-green-100 text-sm">Net GST Payable</p>
                                <p className="text-2xl font-bold">{formatCurrency(totalStats.totalNetPayable)}</p>
                            </CardContent>
                        </Card>
                        <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
                            <CardContent className="p-4">
                                <Receipt className="h-8 w-8 text-purple-200 mb-2" />
                                <p className="text-purple-100 text-sm">Input Tax Credit</p>
                                <p className="text-2xl font-bold">{formatCurrency(totalStats.totalInputTax)}</p>
                            </CardContent>
                        </Card>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                        ...
                                        <div className="text-center p-4 bg-gray-50 rounded-lg">
                                            ...
                                            <p className="text-xl font-bold text-green-600">
                                                {formatCurrency(currentMonthData.inputTax)}
                                            </p>
                                        </div>
                                        <div className="text-center p-4 bg-blue-50 rounded-lg border-2 border-blue-200 col-span-2 md:col-span-1">
                                            <p className="text-sm text-blue-600 font-medium">Net Payable</p>
                                            <p className="text-xl font-bold text-blue-700">
                                                {formatCurrency(currentMonthData.netPayable)}
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                                            ...
                                            <tbody className="divide-y">
                                                {gstData.slice(0, 12).map((g) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4 text-right text-green-600">
                                                            {formatCurrency(g.inputTax)}
                                                        </td>
                                                        <td className="px-6 py-4 text-right font-bold text-blue-600">
                                                            {formatCurrency(g.netPayable)}
                                                        </td>
